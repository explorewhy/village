...
export default {
  ...
  mounted () {
    console.log(this.$store.state.populationChartData);
    if (this.$store.state.populationChartData) {
      this.options.push(
        {
          value: 'all',
          name: '全部城市'
      ...
      for (let i = 0; i < this.$store.state.populationChartData.length; i++) {
        const cities = {
          value: this.$store.state.populationChartData[i].value,
          name: this.$store.state.populationChartData[i].name
        };
        this.options.push(cities);
      }
      // eslint-disable-next-line no-unused-vars
      let allPopulations = 0;
      for (let i = 0; i < this.$store.state.populationChartData.length; i++) {
        allPopulations += this.$store.state.populationChartData[i].value;
      }
      for (let j = 0; j < this.$store.state.populationChartData.length; j++) {
        const data = {
          city: this.$store.state.populationChartData[j].name,
          population: this.$store.state.populationChartData[j].value,
          populationProportion: ((this.$store.state.populationChartData[j].value) / allPopulations) * 100 + '%'
        };
  ...
  methods: {
    ...
    exportExcel () {
      ...
      require.ensure([], () => {
        ...
        const tHeader = ['城市', '人口总量', '人口比例'];
        const filterVal = ['city', 'population', 'populationProportion'];
