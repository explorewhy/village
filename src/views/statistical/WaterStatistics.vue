    ...
    <div class="other">
      ...
      <div class="table-show">
        <el-table
          ...
          <el-table-column
              prop="downWaterSource"
              label="地下水资源量"
              align="center">
          </el-table-column>
          <el-table-column
              prop="downWaterSourceAll"
              label="水资源总量"
              align="center">
          </el-table-column>
        </el-table>
...
export default {
  ...
  mounted () {
    if (this.$store.state.waterSourceData[0]) {
      this.options.push(
        {
          value: 'all',
          name: '全部城市'
      ...
      for (let i = 1; i < this.$store.state.waterSourceData[0].length; i++) {
        const cities = {
          value: this.$store.state.waterSourceData[0][i],
          name: this.$store.state.waterSourceData[0][i]
        };
        this.options.push(cities);
      }
      for (let j = 1; j < this.$store.state.waterSourceData.length; j++) {
        const data = {
          city: this.$store.state.waterSourceData[j][0],
          downWaterSource: this.$store.state.waterSourceData[j][1],
          downWaterSourceAll: this.$store.state.waterSourceData[j][2]
        };
  ...
  methods: {
    ...
    exportExcel () {
      ...
      require.ensure([], () => {
        ...
        const tHeader = ['城市', '地下水资源', '地下水资源总量'];
        const filterVal = ['city', 'downWaterSource', 'downWaterSourceAll'];
