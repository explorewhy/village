  ...
  <div class="water-statistics">
    ...
    <div class="other">
      ...
      <div class="btn">
        ...
        <el-button type="primary" size="mini" @click="foodManager">粮仓管理</el-button>
      </div>
      <!--table表格-->
...
export default {
  ...
  mounted () {
    console.log(this.$store.state.footChartData);
    if (this.$store.state.footChartData.city) {
      this.options.push(
        {
          value: 'all',
          name: '全部城市'
      ...
      for (let i = 0; i < this.$store.state.footChartData.city.length; i++) {
        const cities = {
          value: this.$store.state.footChartData.city[i],
          name: this.$store.state.footChartData.city[i]
        };
        this.options.push(cities);
      }
      for (let j = 0; j < this.$store.state.footChartData.series.length; j++) {
        const data = {
          city: this.$store.state.footChartData.city[j],
          foodProduction: this.$store.state.footChartData.series[j]
        };
  ...
  methods: {
    ...
    exportExcel () {
      ...
      require.ensure([], () => {
        ...
        const tHeader = ['城市', '粮食产量'];
        const filterVal = ['city', 'foodProduction'];
